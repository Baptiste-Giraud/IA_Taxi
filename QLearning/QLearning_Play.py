...
def launch(path: str = "Qlearning/models/qtable.npy",
         slow: bool = False,
         render: bool = False,
         is_loop: bool = False,
...
def play(p_time: int = 0,
            loop: int = 1,
            render: bool = False,
            slow: bool = False,
            path: str = "./QLearning/models/qtable.npy"):
    ...
    if maxrt != None:
        ...
        while datetime.datetime.now() < stop:
            mean_steps, mean_result, total_failed = solve(
                path, mean_steps, mean_result, total_failed, slow,
                render, is_loop, True)
            total += 1
...
if __name__ == "__main__":
    ...
    play(args.time, args.loop, args.render, args.slow, args.path)